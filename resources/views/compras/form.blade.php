...
<div class="row g-3">
    ...
    <div class="col-md-4"><label class="form-label" for="preco">Preço do pacote</label><div class="input-group"><span class="input-group-text">R$</span><input id="preco" name="preco" type="number" min="0" step="0.01" class="form-control" value="{{ old('preco',$compra->preco) }}"></div></div>
...
document.addEventListener('DOMContentLoaded',function(){
    ...
    const calculate=()=>{const total=number('quantidade')*number('quantidade_unitaria');const used=number('qtde_utilizada'),unitPrice=total>0?number('preco')/total:0;document.getElementById('quantidade_total').value=decimal(total);document.getElementById('quantidade_disponivel').value=decimal(total-used);document.getElementById('valor_gasto').value=currency(used*unitPrice);};
